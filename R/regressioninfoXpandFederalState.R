...
mobilityPlot <- ggplot(data=filter(joinedDataFrame, Bundesland=="Berlin"), aes(x=Date, y=outOfHomeDuration))+
  geom_point()+
  theme_minimal()
tempPlot <- ggplot(data=filter(joinedDataFrame, Bundesland=="Berlin"), aes(x=Date, y=outdoorFraction))+
  geom_point() +
  theme_minimal() +
  ylab("Contributing factor of temp")
grid.arrange(changeOfIncidencePlot, mobilityPlot, tempPlot , nrow=3)
